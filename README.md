...
├── assets/
│   ├── css/style.css                   # 스타일시트
│   ├── js/main.js                      # 지도/목록 렌더링, relive 오버레이
...
2. `tracks.json`에 항목 추가 (`path`, `title`, `description`, `date`, `region`, `bike` 등).
   - Relive(relive.cc) 기록이 있으면 `"relive": "https://www.relive.cc/view/..."` 필드를 추가하면
     목록에 🎬 Relive 보기 버튼이 생기고, 클릭 시 지도 위에 오버레이(모달)로 동영상/사진을 보여줌.
